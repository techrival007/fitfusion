...
async def mess_overview(user=require_role("mess_manager", "dean"), db=Depends(get_db)):
    ...
    for r in skip_today:
        if r["meal_type"] and r["skip_pct"] and (1 - r["skip_pct"]) > highest_skip["pct"]:
            pct = round((1 - float(r["skip_pct"])) * 100, 1)
            highest_skip = {"meal": r["meal_type"], "pct": pct,
                            "color": "#E24B4A" if pct > 40 else "#BA7517" if pct > 20 else "#1D9E75"}
    ...
    data = {
        "kpis": {
            ...
            "worst_rated_meal_week": {
                ...
                "rating": round(float(worst_week["avg_rating"] or 0), 2) if worst_week else 0,
            },
        },
        "participation_chart": [
            {"day": str(r["date"]), "breakfast": r["breakfast"] or 0,
             "lunch": r["lunch"] or 0, "snacks": r["snacks"] or 0, "dinner": r["dinner"] or 0}
...
async def mess_ratings(
    ...
    dow_filter  = "" if day_of_week == "all" else f"AND EXTRACT(DOW FROM nl.date) = {int(day_of_week)}"
    ...
    return {
        ...
        "worst_meals": [
            {"meal_name": r["meal_type"], "date": str(r["date"]),
             "rating": round(float(r["rating"] or 0), 2), "complaint_tag": r["complaint_tag"]}
            for r in worst
        ],
        "best_meals": [
            {"meal_name": r["meal_type"], "date": str(r["date"]),
             "rating": round(float(r["rating"] or 0), 2), "positive_tag": r["positive_tag"]}
            for r in best
        ],
        "tag_frequencies": [
            {"tag": r["tag"], "count": r["cnt"],
             "type": "positive" if r["tag"] in POSITIVE_TAGS else "negative"}
            for r in tags if r["tag"]
        ],
    }
...
async def mess_nutrients(
    ...
    if gauges["protein"]["status"] == "deficient":
        recs.append({"nutrient": "protein", "message": "Add dal, paneer or egg options to increase protein content"})
    if gauges["fibre"]["status"] == "deficient":
        recs.append({"nutrient": "fibre", "message": "Include more vegetables and whole grains in the menu"})
    if gauges["calories"]["status"] == "deficient":
        recs.append({"nutrient": "calories", "message": "Students may be undereating — consider increasing portion sizes"})

    return {
        "gauges": gauges,
        "macro_trend": [
            {"week": str(r["week"]), "protein_g": round(float(r["protein_g"] or 0), 1),
             "carbs_g": round(float(r["carbs_g"] or 0), 1), "fat_g": round(float(r["fat_g"] or 0), 1)}
...
async def get_menu(
    ...
    grid  = {d: {m: None for m in MEALS} for d in DAYS}
    daily_totals = {d: {"calories": 0, "protein": 0} for d in DAYS}
    ...
    for r in rows:
        ...
        if meal in MEALS:
            slot = {
                "id": str(r["id"]), "food_items": r["food_items"],
...
async def create_menu_slot(
    ...
    food_rows  = await db.fetch(
        "SELECT * FROM food_items WHERE id = ANY($1::uuid[])", food_ids
    ) if food_ids else []
    ...
    for allergen in allergen_map:
        ...
        if cnt:
            allergen_conflicts.append({"allergen": allergen, "affected_count": int(cnt)})

    row = await db.fetchrow(
        """INSERT INTO mess_menu(week_number, day_of_week, meal_type, food_items,
           estimated_calories, estimated_protein)
           VALUES($1,$2,$3,$4,$5,$6) RETURNING *""",
        body.week_number, body.day_of_week, body.meal_type,
    ...
    return {
        "menu_entry": slot,
        "nutrition_totals": {
            "calories": round(total_cal, 1), "protein": round(total_pro, 1),
...
async def update_menu_slot(
    ...
    for allergen in allergen_map:
        cnt = await db.fetchval(
            "SELECT COUNT(*) FROM users WHERE role='student' AND $1=ANY(allergens)", allergen
        )
        if cnt:
            allergen_conflicts.append({"allergen": allergen, "affected_count": int(cnt)})
    ...
    row = await db.fetchrow(
        ...
        json.dumps(body.food_items), round(total_cal, 1), round(total_pro, 1), menu_id
    )
    if not row:
    ...
    return {
        "menu_entry": slot,
        "nutrition_totals": {"calories": round(total_cal, 1), "protein": round(total_pro, 1)},
        "allergen_conflicts": allergen_conflicts,
    }
...
async def publish_menu(
    ...
    return {"published": True, "week_number": body.week_number, "published_at": datetime.utcnow().isoformat()}
...
async def mess_feedback(
    ...
    return {
        "table": [
            {"date": str(r["date"]), "meal_type": r["meal_type"],
             "avg_rating": round(float(r["avg_rating"] or 0), 2),
             "n_ratings": r["n_ratings"],
             "positive_tags": [t for t in (r["tags"] or []) if t in POSITIVE],
             "negative_tags": [t for t in (r["tags"] or []) if t and t not in POSITIVE]}
            for r in table
        ],
        "ratio_trend": [
            {"week": str(r["week"]),
