...
import { Activity, Home, Utensils, Moon, BookOpen, Award, Phone, LogOut, Zap, Menu, X } from 'lucide-react'
      ...
      <div className="p-5 border-b border-[#E5E7EB]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-[#9CA3AF]">Student</p>
      ...
      <div className="px-4 py-3 border-b border-[#E5E7EB]">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 bg-[#111827] rounded-full flex items-center justify-center shrink-0">
            <span className="text-[9px] font-bold text-white">{user?.name?.split(' ').map(n => n[0]).join('').slice(0, 2)}</span>
          </div>
          <div className="min-w-0">
            <p className="text-[11px] font-bold text-[#111827] truncate">{user?.name}</p>
            <p className="text-[9px] text-[#9CA3AF]">{user?.hostel} · {user?.branch} Y{user?.year}</p>
          </div>
        </div>
        <div className="mt-2 flex items-center gap-2">
          <span className="text-[9px] font-bold uppercase tracking-widest text-[#9CA3AF]">Streak</span>
          <span className="text-[10px] font-bold text-[#111827]">🔥 {user?.streakDays} days</span>
        </div>
      </div>

      <nav className="flex-1 overflow-y-auto py-3 px-3">
        <p className="text-[9px] font-bold uppercase tracking-widest text-[#9CA3AF] px-2 mb-2">Menu</p>
        {NAV.map(item => {
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-2 px-3 py-2 text-[11px] font-medium transition-all duration-150 rounded-lg mb-0.5 ${
                  isActive ? 'bg-white border border-[#E5E7EB] text-[#111827] shadow-sm font-bold' : 'text-[#6B7280] hover:text-[#111827] hover:bg-[#F9FAFB]'
                }`
              }
            >
              <Icon size={13} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="lg:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-[#E5E7EB] shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[9px] text-[#9CA3AF]">🔥 {user?.streakDays}d</span>
