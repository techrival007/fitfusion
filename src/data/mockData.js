...
const gauss = (mean, std, rand) => {
  ...
  return (
    mean +
    std * Math.sqrt(-2 * Math.log(u1 + 0.001)) * Math.cos(2 * Math.PI * u2)
  );
};

const EXAM_DAYS = new Set([
  ...Array.from({ length: 8 }, (_, i) => i + 30),
  ...Array.from({ length: 8 }, (_, i) => i + 70),
]);
...
export function generateHostelWellnessSeries(hostelId, days = 90) {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    return {
      ...
      studentsLogged: Math.floor(
        clamp(gauss(isExam ? 55 : 72, 10, r), 30, 100),
      ),
      isExam,
    };
  });
...
export function getActivityReport(hostelName, days = 28) {
  ...
  return {
    dailyTrend: series.map((d, i) => ({
      ...
      outdoor: clamp(
        Math.round(d.activityMin * (env[i].aqi > 150 ? 0.2 : 0.6)),
        0,
        60,
      ),
      indoor: clamp(
        Math.round(d.activityMin * (env[i].aqi > 150 ? 0.8 : 0.4)),
        0,
        45,
      ),
    })),
    ...
    weeklyAQI: env
      ...
      .map((e, i) => ({
        week: `W${i + 1}`,
        aqi: e.aqi,
        indoor: series[i * 7]?.activityMin
          ? Math.round(series[i * 7].activityMin * 0.45)
          : 18,
        outdoor: series[i * 7]?.activityMin
          ? Math.round(series[i * 7].activityMin * 0.55)
          : 22,
      })),
  };
}
...
export const mealRatingsData = Array.from({ length: 30 }, (_, i) => {
  ...
  return {
    ...
    day: days[d.getDay()],
    breakfast: {
      rating: clamp(gauss(3.6, 0.5, r), 1, 5),
      count: Math.floor(gauss(580, 60, r)),
      tag: "Good",
    },
    lunch: {
      rating: clamp(gauss(3.4, 0.6, r), 1, 5),
      count: Math.floor(gauss(720, 50, r)),
      tag: "Okay",
    },
    snacks: {
      rating: clamp(gauss(3.8, 0.4, r), 1, 5),
      count: Math.floor(gauss(490, 70, r)),
      tag: "Tasty",
    },
    dinner: {
      rating: clamp(gauss(3.1, 0.7, r), 1, 5),
      count: Math.floor(gauss(680, 55, r)),
      tag: "Bland",
    },
  };
});
...
export const campusWeeklyTrend = (() => {
  const weeks = Array.from({ length: 7 }, (_, wi) => {
    ...
    HOSTELS.forEach((h) => {
      ...
      entry[h.name] =
        Math.round(
          (slice.reduce((s, d) => s + d.wellnessScore, 0) / slice.length) * 10,
        ) / 10;
    });
    return entry;
  });
...
export const academicCorrelationData = Array.from({ length: 90 }, (_, i) => {
  const allHostels = HOSTELS.map(
    (h) => hostelWellnessData[h.name][i].wellnessScore,
  );
  ...
  return {
    ...
    moodScore: clamp(
      gauss(environmentalData[i].isExamDay ? 2.8 : 3.3, 0.3, r),
      1,
      5,
    ),
    stressLevel: clamp(
      gauss(environmentalData[i].isExamDay ? 3.8 : 2.4, 0.5, r),
      1,
      5,
    ),
    sleepHours: clamp(
      gauss(environmentalData[i].isExamDay ? 5.7 : 6.8, 0.6, r),
      4,
      9,
    ),
    activityMin: clamp(
      gauss(environmentalData[i].isExamDay ? 28 : 40, 8, r),
      5,
      65,
    ),
    ...
    data: Array.from({ length: 13 }, (_, i) => {
      ...
      return {
        ...
        score: clamp(
          gauss(
            baseScore -
              (i === 5 || i === 11 ? baseScore * (1 - examSensitivity) : 0),
            3,
            r,
          ),
          30,
...
export const adminAlerts = [
  {
    ...
    type: "sleep_deficit",
    severity: "warning",
    title: "Sleep deficit persisting in BH-3",
    detail:
      "Average sleep hours have been 6.1 for the past 6 days (threshold: 6.5 hrs)",
  ...
  },
  {
    ...
    type: "mood_crisis",
    severity: "critical",
    title: "Sustained low mood in GH-2",
    detail: "Average mood score has been below 2.5 for 4 consecutive days",
    triggeredAt: "2026-03-09",
    isActive: true,
    metricValue: 2.3,
    threshold: 2.5,
  },
  {
    ...
    severity: "warning",
    title: "Activity participation has dropped significantly",
    detail: "Only 31% of BH-5 has logged activity in the past 5 days",
  ...
  },
  {
    ...
    type: "environmental",
    severity: "info",
    title: "Outdoor activity not recommended today",
    detail:
      "Current AQI is 187 (Moderate). Advise students to exercise indoors.",
    triggeredAt: "2026-03-13",
    isActive: true,
    metricValue: 187,
    threshold: 150,
  },
  {
    ...
    type: "nutrition_gap",
    severity: "info",
    title: "Students may be undereating this week",
    detail:
      "Average logged calorie intake is 1,520 kcal — below 1,600 kcal threshold for 3 days",
    triggeredAt: "2026-03-11",
    isActive: true,
    metricValue: 1520,
    threshold: 1600,
  },
  {
    ...
    type: "high_stress",
    severity: "warning",
    title: "High stress levels being reported in GH-4",
    detail:
      "54% of mood check-ins this week rated stress as high (threshold: 50%)",
    triggeredAt: "2026-03-08",
    isActive: false,
    acknowledgedAt: "2026-03-09",
    metricValue: 54,
    threshold: 50,
  },
];
...
export const anonymousLeaderboard = HOSTELS.map((h) => ({
  ...
  type: h.type,
  avgScore:
    hostelCurrentScores.find((s) => s.name === h.name)?.weeklyAvgScore ?? 65,
...
export const nudges = [
  {
    ...
    type: "activity",
    message:
      "You haven't logged any activity today. Even a 15-min walk counts!",
    trigger: "no_activity_logged",
  },
  {
    id: "n2",
    type: "nutrition",
    message:
      "You skipped breakfast today. Campus mood data shows skipping breakfast correlates with lower afternoon energy.",
    trigger: "meal_skip",
  },
  {
    id: "n3",
    type: "environmental",
    message:
      "AQI is 187 today — moderate. Consider an indoor workout instead of your usual outdoor run.",
    trigger: "high_aqi",
  },
  {
    id: "n4",
    type: "mental",
    message:
      "Your mood has been lower than your usual average for 2 days. Try a 2-min breathing exercise?",
    trigger: "mood_dip",
  },
  {
    id: "n5",
    type: "sleep",
    message:
      "You've averaged 5.8 hrs sleep this week. Try a consistent 10pm–6am schedule this week.",
...
export const achievements = [
  ...
  },
  {
    id: "ach2",
    title: "14-Day Streak",
    description: "Logged data 14 days in a row",
    earned: true,
    points: 200,
  },
  {
    id: "ach3",
    title: "Wellness Pro",
    description: "Maintained score > 70 for 2 weeks",
    earned: true,
    points: 300,
  },
  {
    ...
    description: "Hit 45min/day for 5 consecutive days",
    earned: false,
    points: 150,
  },
  {
    ...
    description: "Logged all 4 meals for 7 days",
    earned: false,
    points: 200,
  },
  {
    id: "ach6",
    title: "30-Day Streak",
    description: "Logged data 30 days in a row",
    earned: false,
    points: 500,
  },
];
...
export function getWellnessLabel(score) {
  if (score >= 80)
    return { label: "Thriving", color: "#22c55e", bg: "#f0fdf4" };
