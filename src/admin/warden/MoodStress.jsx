...
import { getWardenMood } from "../../api/warden";
...
export default function MoodStress() {
  ...
  const mockSeries = (hostelWellnessData[hostelName] || hostelWellnessData['BH-3'] || []).slice(-30);
  const [series, setSeries] = useState(mockSeries);
  const [weeklyStressApi, setWeeklyStressApi] = useState(null)

  useEffect(() => {
    getWardenMood({ range: '30d' }).then(api => {
      if (api.mood_trend?.length > 0) {
        setSeries(api.mood_trend.map(d => ({ date: d.date, moodAvg: d.avg_mood, isExam: false })))
      }
      if (api.stress_distribution?.length) {
        setWeeklyStressApi(api.stress_distribution.map((row, index) => ({
          week: row.week || `W${index + 1}`,
          low: row.low_pct,
          moderate: row.moderate_pct,
          high: row.high_pct,
        })))
      }
    }).catch(() => {})
  ...
  const weeklyStress = weeklyStressApi || Array.from({ length: 4 }, (_, wi) => {
    const slice = moodLogHistory.slice(wi * 7, (wi + 1) * 7);
    return {
      week: `W${wi + 1}`,
      low: Math.round(
        (slice.filter((d) => d.stressLevel <= 2).length / slice.length) * 100,
      ),
      moderate: Math.round(
        (slice.filter((d) => d.stressLevel === 3).length / slice.length) * 100,
      ),
      high: Math.round(
        (slice.filter((d) => d.stressLevel >= 4).length / slice.length) * 100,
      ),
    };
  });
    ...
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="border border-[#bfdbfe] bg-[#eff6ff] p-4 flex gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-[#1e40af] leading-relaxed">
            Mood data shown here is aggregated across your entire hostel. No
            individual student's mood, energy, or stress scores are visible at
            any point. Individual journal entries are never accessible to any
            admin role. Minimum group size enforced: 30 students.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E5E7EB] p-5">
        <SectionHeader
          ...
          subtitle="Hostel average mood score · 1 = very low, 5 = great"
        />
        ...
        {lowMoodDays >= 3 && (
          <div className="mt-4 border border-[#fde68a] bg-[#fffbeb] p-3">
            <p className="text-[11px] text-[#92400e]">
              Hostel mood has been in the low range for {lowMoodDays} days.
              Consider organizing a social activity or study break event.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white border border-[#E5E7EB] p-5">
        <SectionHeader
          title="30-Day Mood Heatmap"
          subtitle="Cell color = hostel average mood"
        />
        <div className="grid grid-cols-5 sm:grid-cols-10 gap-1.5">
          {moodTrend.map((d, i) => (
            <div
              ...
              style={{
                backgroundColor: getMoodColor(d.mood) + "22",
                borderColor: getMoodColor(d.mood) + "44",
              }}
              title={`${d.date}: ${d.mood.toFixed(2)}`}
            >
              <span className="text-[#111827]">{d.date}</span>
              <span style={{ color: getMoodColor(d.mood) }}>
                {d.mood.toFixed(1)}
              </span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-[#E5E7EB] p-5">
          <SectionHeader
            ...
            subtitle="% of responses by stress level"
          />
        ...
        <div className="bg-white border border-[#E5E7EB] p-5">
          <SectionHeader
            ...
            subtitle="Aggregate mood by time of day"
          />
          <ResponsiveContainer width="100%" height={180}>
            <BarChart
              data={timeOfDayMood}
